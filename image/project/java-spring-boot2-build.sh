...
echo 'create_dump_folder() {' >> target/start.sh
echo '    mkdir -p /home/criu-dump-location' >> target/start.sh
echo '    touch /home/criu-dump-location/dump.log /home/criu-dump-location/restore.log' >> target/start.sh
echo '}' >> target/start.sh
echo "" >> target/start.sh
echo 'run_app() {' >> target/start.sh
echo -n '    exec java $JVM_ARGS -cp /app:/app/lib/* -Djava.security.egd=file:/dev/./urandom ' >> target/start.sh
cat target/dependency/META-INF/MANIFEST.MF | grep 'Start-Class: ' | cut -d' ' -f2 | tr -d '\r\n' >> target/start.sh
echo ' | tee out.log 2>&1 &' >> target/start.sh
echo '}' >> target/start.sh
echo "" >> target/start.sh
echo 'get_app_pid() {' >> target/start.sh
echo '    echo $(ps -ef | grep java | grep -v grep | awk '"'"'{ print $2 }'"'"')' >> target/start.sh
echo '}' >> target/start.sh
echo "" >> target/start.sh
echo 'check_server_started() {' >> target/start.sh
echo '    for i in $(seq 1 100)' >> target/start.sh
echo '    do' >> target/start.sh
echo '        cat /out.log | grep "Tomcat started on port" &> /dev/null' >> target/start.sh
echo '        local init=$?' >> target/start.sh
echo '        if [ ${init} -eq 0 ]; then' >> target/start.sh
echo '            break' >> target/start.sh
echo '        else' >> target/start.sh
echo '            if [ $i -eq 100 ]; then' >> target/start.sh
echo '                exit 1' >> target/start.sh
echo '            fi' >> target/start.sh
echo '            sleep 1' >> target/start.sh
echo '        fi' >> target/start.sh
echo '    done' >> target/start.sh
echo '}' >> target/start.sh
echo "" >> target/start.sh
echo 'initiate_dump() {' >> target/start.sh
echo '    mkdir -p /home/criu-dump-location/dump-image-store' >> target/start.sh
echo '    cd /home/criu-dump-location/dump-image-store' >> target/start.sh
echo '    check_server_started' >> target/start.sh
echo '    if [ $? -eq 0 ]; then' >> target/start.sh
echo '        criu dump -t "$1" --tcp-established -j --leave-running -v4 -o "$2"' >> target/start.sh
echo '        if [ $? -eq 0 ]; then' >> target/start.sh
echo '            rm -f /out.log' >> target/start.sh
echo '            echo "[ checkpoint successfull ]" > /criu-log.txt' >> target/start.sh
echo '            echo "[ checkpoint successfull ]"' >> target/start.sh
echo '        fi' >> target/start.sh
echo '    fi    ' >> target/start.sh
echo '}' >> target/start.sh
echo "" >> target/start.sh
echo 'initialize_restore() {' >> target/start.sh
echo '    cd /home/criu-dump-location/dump-image-store' >> target/start.sh
echo '    criu restore --tcp-established -j -v3 -o "$1"' >> target/start.sh
echo '}' >> target/start.sh
echo "" >> target/start.sh
echo 'remove_dump_folder() {' >> target/start.sh
echo '    rm -rf /home/criu-dump-location' >> target/start.sh
echo '}' >> target/start.sh
echo "" >> target/start.sh
echo 'initiate_checkpoint() {' >> target/start.sh
echo '    remove_dump_folder' >> target/start.sh
echo '    create_dump_folder' >> target/start.sh
echo '    run_app' >> target/start.sh
echo '    app_pid=$(get_app_pid)' >> target/start.sh
echo '    initiate_dump $app_pid "/home/criu-dump-location/dump.log"' >> target/start.sh
echo '}' >> target/start.sh
echo "" >> target/start.sh
echo 'if [ -z $APPSODY_CRIU_ENABLED] || [ $APPSODY_CRIU_ENABLED != "true" ]' >> target/start.sh
echo 'then' >> target/start.sh
echo -n '    exec java $JVM_ARGS -cp /app:/app/lib/* -Djava.security.egd=file:/dev/./urandom ' >> target/start.sh
cat target/dependency/META-INF/MANIFEST.MF | grep 'Start-Class: ' | cut -d' ' -f2 | tr -d '\r\n' >> target/start.sh
echo "" >> target/start.sh
echo 'else' >> target/start.sh
echo '    umount -R /proc' >> target/start.sh
echo '    mount -t proc proc /proc' >> target/start.sh
echo '    ' >> target/start.sh
echo '    if [ -d "/home/criu-dump-location/dump-image-store" ]; then' >> target/start.sh
echo '        initialize_restore "/home/criu-dump-location/restore.log"' >> target/start.sh
echo '    else' >> target/start.sh
echo '        touch /criu-log.txt' >> target/start.sh
echo '        initiate_checkpoint' >> target/start.sh
echo '    fi' >> target/start.sh
echo 'fi' >> target/start.sh
echo "" >> target/start.sh
